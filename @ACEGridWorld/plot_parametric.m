...
% Seperate plot for comparison
    crmi_path = [494	469	443	417	441	415	389	365	341	317	293	268	243	219	194	170	145	121	96	70	44];
    true_path = [494	493	492	466	440	415	389	365	341	317	293	268	243	219	194	170	145	121	96	70	44];
    smi_path  = [494	469	444	420	396	372	348	324	300	275	250	225	200	174	148	122	96	70	44];
    if flags_.SHOW_PATH
			  plot3(...
				   obj.coordinates(1, crmi_path), ...
				   obj.coordinates(2, crmi_path), ...
				    imageMax*ones(1, length(crmi_path)), ...
				    'o', 'Color', 'r', 'MarkerSize', 10, 'LineWidth', 1.3); hold on
                    plot3(...
				    obj.coordinates(1, true_path), ...
				    obj.coordinates(2, true_path), ...
				    imageMax*ones(1, length(true_path)), ...
				    'hexagram', 'Color', 'k', 'MarkerSize', 8, 'LineWidth', 1.3);
                    plot3(...
				    obj.coordinates(1, smi_path ), ...
				    obj.coordinates(2, smi_path ), ...
				    imageMax*ones(1, length(smi_path)), ...
				    'v', 'Color', 'm', 'MarkerSize', 8, 'LineWidth', 1.3);
                    text(obj.coordinates(1, 494)+0.02, obj.coordinates(2,494)+0.08,2*imageMax, 'S', 'FontName', 'Times New Roman', ...
	                    'FontSize', 24, 'Color', 'r');
                    text(obj.coordinates(1, 44)-.12, obj.coordinates(2, 44)-.12, 2*imageMax, 'G', 'FontName', 'Times New Roman', ...
	                    'FontSize', 24, 'Color', 'r');
